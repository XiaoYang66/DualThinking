...
from inference.deepseek import request_deepseek  # think mode using deepseek
...
def inference_sysytem1_2(question: dict, enable_thinking, args):
    ...
    if "Qwen" in args.model_name:
        # If the model is Qwen, use the request_qwen function
        request_func = request_qwen
    else:
        # If the model is not Qwen, you can define another request function here
        # For now, we will just use the same function as a placeholder
        raise NotImplementedError("Only Qwen model is supported in this script.")
