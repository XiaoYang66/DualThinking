...
from inference.deepseek_distill import request_distill  # think mode using deepseek
...
def inference_sysytem1_2(question: dict, enable_thinking, args):
    ...
    if "Distill" in args.model_name:
        # If the model is Distill, use the request_distill function
        request_func = request_distill
    else:
        # If the model is not Distill, you can define another request function here
        # For now, we will just use the same function as a placeholder
        raise NotImplementedError("Only Distill model is supported in this script.")
