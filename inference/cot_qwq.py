...
from inference.qwq import request_qwq  # think mode using deepseek
...
def inference_sysytem1_2(question: dict, enable_thinking, args):
    ...
    assert enable_thinking, "QwQ model only supports thinking mode."
    ...
    if "QwQ" in args.model_name:
        # If the model is QwQ, use the request_qwq function
        request_func = request_qwq
    else:
        # If the model is not QwQ, you can define another request function here
        # For now, we will just use the same function as a placeholder
        raise NotImplementedError("Only QwQ model is supported in this script.")
