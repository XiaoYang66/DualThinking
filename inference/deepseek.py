"""
DeepSeek-compatible client wrappers.

Environment variables (optional):
- DEEPSEEK_BASE:     default "http://localhost:50073/v1"
- DEEPSEEK_TOKENIZE: default "http://localhost:50073/tokenize"
- DEEPSEEK_OPENAI_API_KEY: default "None"
"""
...
OPENAI_API_BASE_THINKING = os.getenv("DEEPSEEK_BASE", "http://localhost:50073/v1")
OPENAI_API_BASE_THINKING_TOKENIZED = os.getenv("DEEPSEEK_TOKENIZE", "http://localhost:50073/tokenize")
OPENAI_API_KEY = os.getenv("DEEPSEEK_OPENAI_API_KEY", "None")  # Not used by vLLM

DEEPSEEK_MAX_TOKENS = 131072
...
def _make_request_tokenize_count(
    ...
    # get model name
    base_url = OPENAI_API_BASE_THINKING
    client = OpenAI(
        base_url=base_url,
    ...
    base_url = OPENAI_API_BASE_THINKING_TOKENIZED
...
def _make_request_response(
    ...
    logger.info(
        "Requesting DeepSeek R1, max_tokens=%s, temperature=%s, top_p=%s, top_k=%s, seed=%s",
        max_tokens, temperature, top_p, top_k, sampling_seed,
    )
...
def _make_request(
    ...
    """
    count_tokens_only = False  # 强制为False，因为DeepSeek不支持计数
...
def request_deepseek(
    ...
    print(
        f"Requesting deepseek with thinking={thinking}, max_tokens={max_tokens}, temperature={temperature}, top_p={top_p}, top_k={top_k}, sampling_seed={sampling_seed}"
    )
    ...
    #     print(
    #         f"Warning: The total token count ({token_count + max_tokens}) exceeds the maximum limit ({QWEN_MAX_TOKENS})."
    #     )
